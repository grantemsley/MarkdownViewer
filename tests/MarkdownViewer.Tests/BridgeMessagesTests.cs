...
public class BridgeMessagesTests
{
    ...
    [Fact]
    public void MarkdownDoc_SerializesCamelCase_WithTypeAndKind()
    {
        var el = Roundtrip(new MarkdownDocMsg(
            @"C:\v\a.md", "https://app.local/__vault/", "<h1>x</h1>",
    ...
    {
        var srcdoc = Roundtrip(new RawDocMsg("a.html", Html: "<p>hi</p>", Url: null, Modified: ""));
        Assert.Equal("<p>hi</p>", srcdoc.GetProperty("html").GetString());
        Assert.False(srcdoc.TryGetProperty("url", out _));

        var byUrl = Roundtrip(new RawDocMsg("a.pdf", Html: null,
            Url: "https://app.local/__vault/a.pdf", Modified: ""));
        Assert.False(byUrl.TryGetProperty("html", out _));
        Assert.Equal("https://app.local/__vault/a.pdf", byUrl.GetProperty("url").GetString());
    }

    [Fact]
    public void OtherDocKinds_CarryTheirKindTag()
    {
        Assert.Equal("text", Roundtrip(new TextDocMsg("a.txt", "python", "x", 0, ""))
            .GetProperty("kind").GetString());
        Assert.Equal("image", Roundtrip(new ImageDocMsg("a.png", "u", ""))
            .GetProperty("kind").GetString());
        Assert.Equal("binary", Roundtrip(new BinaryDocMsg("a.bin", ""))
            .GetProperty("kind").GetString());
        Assert.Equal("empty", Roundtrip(new EmptyDocMsg("Open a folder."))
            .GetProperty("kind").GetString());
        Assert.Equal("setPrefs", Roundtrip(new PrefsMsg("dark", "#FF0000", "system", 14, 85, false, "win11"))
            .GetProperty("type").GetString());
        Assert.Equal("scrollToHeading", Roundtrip(new ScrollToHeadingMsg("h-1"))
            .GetProperty("type").GetString());
    }
    ...
    public void Parse_Scroll()
    {
        var m = Assert.IsType<ScrollMsg>(BridgeInbound.Parse(
            """{"type":"scroll","top":123.5,"path":"C:\\v\\a.md"}""", out var err));
        Assert.Null(err);
    ...
    [InlineData("""{"type":"scroll","path":"x"}""")]        // missing top
    [InlineData("""{"type":"scroll","top":"NaN","path":"x"}""")]
