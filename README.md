...
- 📍 **Place marker** — click in the margin left of the text to drop a "I stopped here" bar on that paragraph or list item (click again to clear, `Ctrl+G` to jump back). Inside a code block — including a script or text file shown in the code viewer — the mark lands on the single line you clicked beside. One per file, shared across tabs, and it stays on the same spot when the file is edited or reloaded from disk. In-memory only: it doesn't survive closing the app.
