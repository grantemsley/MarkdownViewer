...
| 💡 | "Check for updates now" action in Preferences (bypasses the 24h throttle; would also give the installed copy a manual apply entry point besides the banner) · `plans/finished/velopack-updates.md` | 2026-07-18 |
| 💡 | Distinct UIA name for the banner dismiss button (`AutomationProperties.Name="Dismiss update"`): every tab-close button is also named "✕", which hurts screen readers and automation targeting alike (bit during P4 verification) · `plans/finished/velopack-updates.md` | 2026-07-18 |
| 💡 | Move the WebView2 cache out of `%LocalAppData%\MarkdownViewer` (now the Velopack install root, wiped on install/uninstall) to e.g. `%LocalAppData%\MarkdownViewer.Cache`; harmless today (cache regenerates) but the shared folder is a latent foot-gun · `plans/finished/velopack-updates.md` | 2026-07-18 |
